    ...
    <script>
        ...
        function publishToSolr() {
            ...
            if (r == true) {
                $.ajax({
                    type: "POST",
                    url: "http://35.162.165.138:8983/solr/explo/dataimport?command=full-import&indent=on&wt=json",
                    data: "",
                    contentType: false,
                    processData: false,
        ...
        function convertEads() {
            ...
            if (r == true) {
                $.ajax({
                    type: "POST",
                    url: "http://localhost:9090/exploro/?c=explr&m=converteads",
                    data: "",
                    contentType: false,
                    processData: false,
                    success: function (message) {
        ...
        function updateEads() {
            ...
            $.ajax({
                type: "POST",
                url: "http://localhost:9090/exploro/?c=explr&m=formatEads",
                data: "",
                contentType: false,
                processData: false,
                success: function (message) {
                    if (message > 0) {
                        $('#requestStatus').empty();
                        $('#requestStatus').show().css('background', '#66cc00').append("Successfully formatted - " + message + " files").delay(3000).fadeOut();


                    } else {
                        $('#requestStatus').empty();
                        $('#requestStatus').show().css('background', '#b31b1b').append("Failed to convert").delay(3000).fadeOut().empty().delay(4000);
                        // $('#requestStatus').empty();
